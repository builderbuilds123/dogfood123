...
export function MessageInput({ onSendText, onAttachImage, onToggleAudio, isRecording, disabled }: MessageInputProps) {
  ...
  return (
    <div className="chat-form-container">
      <form onSubmit={handleSubmit} className="flex items-end gap-2 chat-input-row">
        {/* Image attach button */}
        <button
          type="button"
          onClick={onAttachImage}
          disabled={disabled}
          className="chat-button-attach shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-surface-light border border-border hover:border-neon-violet/40 transition-colors disabled:opacity-50 min-w-[44px] min-h-[44px]"
          title="Attach image"
        >
        ...
        <div className="chat-buttons-left">
          <button
            ...
            disabled={disabled}
            className={`chat-button-audio shrink-0 w-10 h-10 flex items-center justify-center rounded-full border transition-colors disabled:opacity-50 min-w-[44px] min-h-[44px] ${
              isRecording
        ...
        {/* Send button - maintains 44x44px minimum touch target */}
        <button
          type="submit"
          disabled={disabled || !text.trim()}
          className="shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-neon-violet/80 hover:bg-neon-violet transition-colors disabled:opacity-30 disabled:cursor-not-allowed hover:shadow-[0_0_15px_rgba(139,0,255,0.4)] min-w-[44px] min-h-[44px]"
          title="Send"
        >
